...
// unencryptedHTTP2 returns the protocol set accepting both HTTP/1 and prior-knowledge HTTP/2.
...
func newConnectBackend(t *testing.T, accept bool) *connectBackend {
	...
	backend.url, err = url.Parse("http://" + ln.Addr().String())
	require.NoError(t, err)
...
func serveProxy(t *testing.T, target *url.URL) string {
	...
	srv := &http.Server{Handler: handler, Protocols: unencryptedHTTP2()}
...
// TestConnect_HTTP2_TunnelEstablished covers the only configuration that tunnels today:
// an HTTP/2 frontend forwarding to an HTTP/1 backend.
...
// TestConnect_HTTP2_RefusedTunnelDropsPayload is the regression test for the smuggling vector:
// a backend that refuses the CONNECT must never receive a single payload byte.
